package app.jg.og.zamong.constant;

import app.jg.og.zamong.entity.dream.DreamQuality;
import app.jg.og.zamong.entity.dream.DreamType;

public class DreamConstant {

    public static final String TITLE = "title";
    public static final String CONTENT = "content";

    public static final DreamQuality QUALITY = DreamQuality.SO;
    public static final DreamType TYPE = DreamType.LCD;
    public static final Boolean IS_SHARED = false;
    public static final Integer SLEEP_TIME = 4;
}
